...
restart_service_checked() {
  ...
  if ! run_cmd systemctl restart "$service"; then
    warn "${label} restart failed; showing recent logs."
    show_service_logs "$service"
    return 1
  fi
  if ! run_cmd systemctl is-active --quiet "$service"; then
...
central_component_detected() {
  ...
  if command -v systemctl >/dev/null 2>&1; then
    for service in elasticsearch kibana wazuh-manager; do
      if systemctl list-unit-files --no-legend 2>/dev/null | awk '{print $1}' | grep -x "${service}.service" > /dev/null; then
        detected=1
      fi
...
install_suricata() {
  ...
  if command -v suricata-update >/dev/null 2>&1; then
    if is_test_mode; then
      run_cmd suricata-update || warn "suricata-update failed; continuing with packaged/default rules."
    else
...
EOF
  ...
  if run_cmd systemctl list-unit-files --no-legend | awk '{print $1}' | grep -x 'falco-modern-bpf.service' > /dev/null; then
    FALCO_SERVICE="falco-modern-bpf.service"
  elif run_cmd systemctl list-unit-files --no-legend | awk '{print $1}' | grep -x 'falco-bpf.service' > /dev/null; then
    FALCO_SERVICE="falco-bpf.service"
  elif run_cmd systemctl list-unit-files --no-legend | awk '{print $1}' | grep -x 'falco-kmod.service' > /dev/null; then
    FALCO_SERVICE="falco-kmod.service"
  elif run_cmd systemctl list-unit-files --no-legend | awk '{print $1}' | grep -x 'falco.service' > /dev/null; then
...
get_wazuh_agent_id() {
  local agent_id=""
  # Try local client.keys first (format: "ID NAME IP KEY" space-separated)
  local client_keys
  client_keys="$(sandbox_path /var/ossec/etc/client.keys)"
  if [[ -f "$client_keys" ]]; then
    agent_id="$(awk '{print $1}' "$client_keys" | head -n1 | tr -d ' ')"
  fi
