...
EOF
  ...
  cd /usr/share/kibana
  sudo -u kibana /usr/share/kibana/bin/kibana-plugin install --batch https://packages.wazuh.com/4.x/ui/kibana/wazuh_kibana-4.5.3_7.17.13-1.zip
...
if [ "$existing_wazuh" = true ]; then
  if [[ "${ARIA_ASSUME_YES:-0}" == "1" ]]; then
    remove_existing="y"
    echo "Existing Wazuh installation found; ARIA_ASSUME_YES=1 so it will be removed."
  else
