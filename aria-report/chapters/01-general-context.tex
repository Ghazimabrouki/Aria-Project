...
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.07\textwidth}>{\RaggedRight\arraybackslash}m{0.13\textwidth}>{\RaggedRight\arraybackslash}m{0.17\textwidth}>{\RaggedRight\arraybackslash}m{0.44\textwidth}>{\centering\arraybackslash}m{0.10\textwidth}@{}}
...
\endhead
\midrule
\multicolumn{5}{r}{\small\textit{Continued on the next page}} \\
\endfoot
\bottomrule
\endlastfoot
...
\hyphenpenalty=10000
\exhyphenpenalty=10000
\newcommand{\backlogdesc}[2]{#1\par\vspace{1.5pt}\textbf{Acceptance:} #2}
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.06\textwidth}>{\RaggedRight\arraybackslash}m{0.16\textwidth}>{\RaggedRight\arraybackslash}m{0.54\textwidth}>{\centering\arraybackslash}m{0.08\textwidth}>{\centering\arraybackslash}m{0.10\textwidth}@{}}
...
\midrule
\endfirsthead
\caption[]{Product backlog with sprint mapping and acceptance criteria (continued)}\\
\toprule
\textbf{ID} & \textbf{Feature} & \textbf{Description / Acceptance criteria} & \textbf{Priority} & \textbf{Sprint(s)} \\
\midrule
\endhead
...
\bottomrule
\endlastfoot
PB-01 & Monitoring foundation & \backlogdesc{Install and stabilize the central security and observability stack on cloud infrastructure.}{all services reachable; Kibana shows indices; health checks pass.} & High & S2--S3 \\
PB-02 & Alert ingestion & \backlogdesc{Poll Elasticsearch sources and persist normalized alerts.}{cursor advances; no duplicate processing; normalized alerts stored in SQLite.} & High & S5 \\
PB-03 & Alert enrichment & \backlogdesc{Add GeoIP, MITRE, Sigma, severity, IOC, and whitelist logic.}{enrichment fields populated; whitelist suppresses false positives.} & High & S6 \\
PB-04 & Incident correlation & \backlogdesc{Group related alerts into incidents and investigations.}{correlation rules configurable; related alerts appear under one incident.} & High & S6 \\
PB-05 & AI response & \backlogdesc{Generate summary, narrative, risk assessment, and playbook.}{investigation contains all fields; malformed LLM output handled gracefully.} & High & S7 \\
PB-06 & Analyst workflow & \backlogdesc{Approve, decline, edit, execute, archive, and audit investigations.}{state machine enforces allowed transitions; audit records persisted.} & High & S8--S9 \\
PB-07 & Safe remediation & \backlogdesc{Validate playbooks, protect dangerous actions, execute via Ansible, verify fixes.}{admin-secret gate; dry-run available; execution log and verification saved.} & High & S9 \\
PB-08 & Dashboard & \backlogdesc{Provide alerts, incidents, investigations, assistant, IPS, metrics, and settings pages.}{all pages render; API checks pass; production build succeeds.} & High & S8, S10 \\
PB-09 & Runtime remediation & \backlogdesc{Diagnose runtime findings and staged remediation actions.}{observe cases never produce corrective playbooks; remediation requires explicit approval.} & Medium & S10 \\
PB-10 & Infrastructure monitoring & \backlogdesc{Poll Telegraf metrics and detect anomalies.}{metrics ingested; anomaly rules trigger investigations.} & Medium & S10 \\
PB-11 & Multi-server onboarding & \backlogdesc{Add asset registry, bootstrap monitored servers, and scope data by server.}{new asset added; bootstrap installs required services; endpoints filter by \codepath{asset\_id}.} & Medium & S11 \\
PB-12 & Validation & \backlogdesc{Compile, test, build, and document verification results.}{automated tests pass; build checks green; evidence screenshots collected.} & High & S12--S13 \\
\end{longtable}
\par}
