...
The internship followed an Agile iterative methodology with two-week sprints. Each sprint delivered a usable increment, followed by a review and planning session for the next. The internship logbook records weekly work from January~12 to July~12, 2026.
...
The methodology was chosen after comparing common project organization models. Waterfall was inappropriate because the monitoring stack, backend workflows, and validation evidence evolved as the environment became available. A full Scrum process was also too heavy for a single-intern implementation context. The adopted approach was therefore a phased iterative method with two-week sprints: each sprint produced a usable technical base, and later sprints refined and extended it.
...
Scrum & Strong feedback rhythm and backlog refinement. & Heavy ceremonies for a compact internship and single main developer. & Used only as inspiration for iterative validation \cite{scrum_guide}. \\
...
The product backlog below summarizes the features prioritized and implemented across these phases, which serve as the working scope and are detailed in Chapters~3 to~5.
...
The complete planning is summarized in the Gantt chart of Figure~\ref{fig:gantt}, spanning January~12 to July~12, 2026. The chart shows how the work packages overlap: the monitoring foundation begins while cloud preparation finishes, the backend intelligence layer builds on the available telemetry, the dashboard and multi-server work extend through the middle of the internship, and validation and documentation run continuously until the final weeks.
