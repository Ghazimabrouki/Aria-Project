...
\begin{adjustbox}{max width=\textwidth}\begin{tikzpicture}[
  font=\small,
  src/.style={draw, rounded corners, align=center, text width=2.4cm, minimum height=1.25cm, fill=gray!8, font=\fontsize{10}{12}\selectfont},
  fwd/.style={draw, rounded corners, align=center, text width=2.3cm, minimum height=1.25cm, fill=teal!10, font=\fontsize{10}{12}\selectfont},
  store/.style={draw, cylinder, shape border rotate=90, aspect=0.25, align=center, text width=2.5cm, minimum height=1.5cm, fill=blue!8, font=\fontsize{10}{12}\selectfont},
  app/.style={draw, rounded corners, align=center, text width=2.4cm, minimum height=1.25cm, fill=orange!10, font=\fontsize{10}{12}\selectfont},
...
% Source nodes
\node[src] (wazuh) {\includegraphics[width=0.5cm]{assets/logos/wazuh.png}\\[2pt]\textbf{Wazuh}\\host security};
\node[src, below=0.35cm of wazuh] (suricata) {\includegraphics[width=0.5cm]{assets/logos/suricata.png}\\[2pt]\textbf{Suricata}\\network IDS};
\node[src, below=0.35cm of suricata] (falco) {\includegraphics[width=0.5cm]{assets/logos/falco.png}\\[2pt]\textbf{Falco}\\runtime security};
\node[src, below=0.35cm of falco] (telegraf) {\includegraphics[width=0.5cm]{assets/logos/influxdb.png}\\[2pt]\textbf{Telegraf}\\metrics};

% Forwarder
\node[fwd, right=1.2cm of suricata] (filebeat) {\includegraphics[width=0.5cm]{assets/logos/filebeat.png}\\[2pt]\textbf{Filebeat}\\+ Falcosidekick};

% Store
\node[store, right=1.2cm of filebeat, yshift=-0.5cm] (es) {\includegraphics[width=0.5cm]{assets/logos/elasticsearch.png}\\[2pt]\textbf{Elasticsearch}\\indices};

% App nodes
\node[app, above=0.9cm of es] (kibana) {\includegraphics[width=0.5cm]{assets/logos/elasticstack.png}\\[2pt]\textbf{Kibana}\\explore};
\node[app, right=1.2cm of es] (aria) {\includegraphics[width=0.5cm]{assets/logos/aria-logo.png}\\[2pt]\textbf{ARIA}\\backend};
\node[store, below=0.9cm of aria] (redis) {\includegraphics[width=0.5cm]{assets/logos/redis.png}\\[2pt]\textbf{Redis}\\cache/cursors};
...
\raisebox{-0.18cm}{\includegraphics[width=0.8cm]{assets/logos/suricata.png}}\hspace{0.15cm}Suricata is the network intrusion detection engine. It inspects traffic on a network interface and emits structured security events. On the central host, \sourcefile{suricata_setup.sh} installs Suricata from the official OISF stable PPA together with its capture dependencies, then performs a key piece of automation: it detects the active network interface from the default route (\codepath{ip route show default}) and writes it into both \codepath{/etc/default/suricata} and the rendered configuration. This avoids the most common Suricata misconfiguration, which is listening on the wrong interface and therefore seeing no traffic.
...
\raisebox{-0.18cm}{\includegraphics[width=0.8cm]{assets/logos/influxdb.png}}\hspace{0.15cm}Telegraf is the metrics agent. It collects infrastructure metrics such as CPU, memory, disk, network, load, processes, and sockets, and writes them to Elasticsearch. On the central host, \sourcefile{telegraf_setup.sh} installs Telegraf from the InfluxData repository and configures an Elasticsearch output rather than the default InfluxDB output, because in this project Elasticsearch is the single shared store for both security events and metrics.
