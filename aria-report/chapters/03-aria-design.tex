...
The previous chapter ended with real telemetry sitting in Elasticsearch. On its own, that telemetry is still a stream of heterogeneous technical documents. ARIA is the platform that gives this data operational meaning.

This chapter explains how ARIA is designed and, more importantly, the \emph{logic} behind each transformation. The goal is to show \emph{why} each stage exists and what problem it solves, not merely to list the modules involved. The discussion is organized in three layers:

\begin{enumerate}[leftmargin=1cm]
\item \textbf{Architecture:} global, backend, frontend, and persistence views of the platform.
\item \textbf{Pipeline:} the path from alert ingestion through normalization, enrichment, deduplication, correlation, investigation, AI-assisted analysis, controlled remediation, verification, and asset scoping.
\item \textbf{Controls:} the security mechanisms and approval boundaries that keep the analyst in control of risky actions.
\end{enumerate}

For each stage we explain not only what happens, but why it is necessary.

\section{Project Repository and Development Environment}

\raisebox{-0.22cm}{\includegraphics[width=1.0cm]{assets/logos/github.png}}\hspace{0.15cm}The entire ARIA project --- source code, setup scripts, Dockerfiles, Ansible playbooks, documentation, tests, and this report --- is maintained in a single GitHub repository at \url{https://github.com/Ghazimabrouki/Aria-Project}. GitHub is used as the version-control platform, the central source of truth, and the distribution point for the project assets. The repository structure separates the monitoring-tool installers (\sourcefile{aria-tools-setup/}), the containerized application (\sourcefile{aria-application/}), the Ansible automation (\sourcefile{ansible-vm-setup/}), the report and presentation (\sourcefile{aria-report/}), and operational scripts (\sourcefile{scripts/}). This organization makes it possible to clone one repository and deploy the full stack, from the central monitoring foundation to the dashboard and to every monitored server.
...
Version control is used throughout the internship. Each significant change --- report chapters, setup scripts, frontend pages, backend routes, diagrams, and validation evidence --- is committed with a descriptive message. The active branch for the report and presentation work is \codepath{report-and-presentation-updates}. This discipline ensures that the deployment artifacts, the validation evidence, and the written report remain synchronized and reproducible.
...
\raisebox{-0.22cm}{\includegraphics[width=1.0cm]{assets/logos/nextjs.png}}\hspace{0.15cm}The dashboard is a Next.js, React, and TypeScript application that uses Tailwind CSS and shadcn/ui for the interface, SWR for data fetching and caching, Recharts for charts, and a WebSocket context for live updates. Its design goal is a dense, SOC-oriented interface: dark mode, severity badges, data tables, live indicators, page skeletons, and a command menu.
...
\item \textbf{Structured output parsing.} The response is expected to contain a summary, a narrative, a risk assessment, and a playbook. The parser extracts these into the \codepath{Investigation} model, and fallback parsing plus rule-based generation handle malformed LLM output.
\end{enumerate}
