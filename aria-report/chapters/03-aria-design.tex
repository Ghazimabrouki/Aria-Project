...
\centering
\begin{adjustbox}{max width=\textwidth}\begin{tikzpicture}[
  lifeline/.style={draw, minimum width=2.5cm, minimum height=0.7cm, fill=gray!8},
  msg/.style={-Latex, thick},
  node distance=1cm
]
\node[lifeline] (analyst) {Analyst};
\node[lifeline, right=1cm of analyst] (api) {FastAPI};
\node[lifeline, right=1cm of api] (ai) {AI Engine};
\node[lifeline, right=1cm of ai] (ansible) {Ansible};
\node[lifeline, right=1cm of ansible] (verify) {Verifier};
\draw[msg] (analyst) -- node[above]{open investigation} (api);
\draw[msg] (api) -- node[above]{build context} (ai);
\draw[msg] (ai) -- node[above]{summary + playbook} (api);
\draw[msg] (api) -- node[above]{await approval} (analyst);
\draw[msg] (analyst) -- node[below]{approve/edit} (api);
\draw[msg] (api) -- node[above]{run playbook} (ansible);
\draw[msg] (ansible) -- node[below]{execution result} (api);
\draw[msg] (api) -- node[above]{verify recurrence/state} (verify);
\draw[msg] (verify) -- node[below]{fixed or unresolved} (api);
\end{tikzpicture}\end{adjustbox}
...
\node[box, right=0.7cm of prompt] (ai) {AI summary and proposed action};
\node[guard, right=0.7cm of ai] (preview) {Preview, validation, risk checks};
\node[guard, below=0.8cm of preview] (approval) {Approval and admin-secret gate};
\node[box, left=0.7cm of approval] (exec) {Ansible execution on selected asset};
\node[box, left=0.7cm of exec] (verify) {Verification and archive};
\draw[arrow] (prompt) -- (ai);
\draw[arrow] (ai) -- (preview);
\draw[arrow] (preview) -- (approval);
