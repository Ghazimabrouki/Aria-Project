...
The previous chapter left real telemetry in Elasticsearch. On its own, that telemetry is a stream of heterogeneous technical documents. ARIA gives it operational meaning.

This chapter explains ARIA's design and the \emph{logic} behind each transformation --- \emph{why} each stage exists and what problem it solves, not merely what modules are involved. The discussion is organized in three layers:

\begin{enumerate}[leftmargin=1cm]
\item \textbf{Architecture:} global, backend, frontend, and persistence views.
\item \textbf{Pipeline:} from alert ingestion through normalization, enrichment, deduplication, correlation, investigation, AI-assisted analysis, controlled remediation, verification, and asset scoping.
\item \textbf{Controls:} the security mechanisms and approval boundaries that keep the analyst in control of risky actions.
\end{enumerate}

\section{Project Repository and Development Environment}

\raisebox{-0.22cm}{\includegraphics[width=1.0cm]{assets/logos/github.png}}\hspace{0.15cm}The entire ARIA project --- source code, setup scripts, Dockerfiles, Ansible playbooks, documentation, tests, and this report --- lives in one GitHub repository at \url{https://github.com/Ghazimabrouki/Aria-Project}. GitHub serves as version-control platform, source of truth, and distribution point. The repository separates monitoring-tool installers (\sourcefile{aria-tools-setup/}), the containerized application (\sourcefile{aria-application/}), Ansible automation (\sourcefile{ansible-vm-setup/}), the report and presentation (\sourcefile{aria-report/}), and operational scripts (\sourcefile{scripts/}). One clone deploys the full stack from central monitoring to dashboard and monitored servers.

\begin{figure}[H]
\centering
\begin{adjustbox}{max width=\textwidth}\begin{tikzpicture}[
  font=\small,
  cat/.style={draw, rounded corners, align=center, text width=3.0cm, minimum height=4.6cm, fill=gray!6},
  item/.style={draw, rounded corners, align=center, text width=2.5cm, minimum height=0.9cm, fill=white, font=\fontsize{9}{11}\selectfont},
  title/.style={font=\fontsize{12}{14}\selectfont\bfseries},
  arrow/.style={-Latex, line width=1.1pt}
]
% Category backgrounds
\node[cat] (os) at (0,0) {};
\node[cat] (sec) at (3.6,0) {};
\node[cat] (data) at (7.2,0) {};
\node[cat] (app) at (10.8,0) {};
\node[cat] (dev) at (14.4,0) {};

% Titles
\node[title, above=0.15cm of os.north] {OS/Cloud};
\node[title, above=0.15cm of sec.north] {Security};
\node[title, above=0.15cm of data.north] {Data/Store};
\node[title, above=0.15cm of app.north] {Application};
\node[title, above=0.15cm of dev.north] {DevOps};

% Items
\node[item] at (0,1.5) {\includegraphics[width=0.45cm]{assets/logos/Huawei-Logo.png}\\Huawei Cloud};
\node[item] at (0,0.25) {\includegraphics[width=0.45cm]{assets/logos/linux.png}\\Linux/Ubuntu};

\node[item] at (3.6,1.5) {\includegraphics[width=0.45cm]{assets/logos/wazuh.png}\\Wazuh};
\node[item] at (3.6,0.25) {\includegraphics[width=0.45cm]{assets/logos/suricata.png}\\Suricata};
\node[item] at (3.6,-0.95) {\includegraphics[width=0.45cm]{assets/logos/falco.png}\\Falco};

\node[item] at (7.2,1.5) {\includegraphics[width=0.45cm]{assets/logos/elasticsearch.png}\\Elasticsearch};
\node[item] at (7.2,0.25) {\includegraphics[width=0.45cm]{assets/logos/elasticstack.png}\\Kibana};
\node[item] at (7.2,-0.95) {\includegraphics[width=0.45cm]{assets/logos/redis.png}\\Redis};

\node[item] at (10.8,1.5) {\includegraphics[width=0.45cm]{assets/logos/python.png}\\Python/FastAPI};
\node[item] at (10.8,0.25) {\includegraphics[width=0.45cm]{assets/logos/nextjs.png}\\Next.js};
\node[item] at (10.8,-0.95) {\includegraphics[width=0.45cm]{assets/logos/influxdb.png}\\Telegraf};

\node[item] at (14.4,1.5) {\includegraphics[width=0.45cm]{assets/logos/docker.png}\\Docker};
\node[item] at (14.4,0.25) {\includegraphics[width=0.45cm]{assets/logos/ansible.png}\\Ansible};
\node[item] at (14.4,-0.95) {\includegraphics[width=0.45cm]{assets/logos/github.png}\\GitHub};
\end{tikzpicture}\end{adjustbox}
\caption{ARIA technology stack grouped by role}\label{fig:tech-stack}
\end{figure}

Version control is used throughout the internship. Each significant change --- report chapters, setup scripts, frontend pages, backend routes, diagrams, and validation evidence --- is committed with a descriptive message on the \codepath{report-and-presentation-updates} branch. This keeps deployment artifacts, validation evidence, and the written report synchronized and reproducible.

\section{Twelve-Step Intelligence Pipeline}

The ARIA workflow is a twelve-step pipeline that carries a signal from raw Elasticsearch document to closed, auditable case. Figure~\ref{fig:twelve-step} gives the high-level view; the sections below explain each stage.
...
\begin{adjustbox}{max width=\textwidth}\begin{tikzpicture}[
  font=\small,
  stepbox/.style={draw, rounded corners, align=center, minimum width=2.4cm, minimum height=0.9cm, fill=white, font=\fontsize{12}{14}\selectfont},
  phase/.style={draw=none, rounded corners=3pt, inner sep=8pt, minimum width=3.1cm},
  arrow/.style={-Latex, line width=1.1pt}
]
\newcommand{\stepnum}[1]{\fontsize{9}{10}\selectfont\textbf{\textcolor{gray!70}{#1}}}
% Phase backgrounds
\node[phase, fill=blue!8, fit={(0,0) (3.1,-4.2)}] (p1) {};
\node[phase, fill=teal!8, fit={(3.6,0) (6.7,-4.2)}] (p2) {};
\node[phase, fill=orange!8, fit={(7.2,0) (10.3,-4.2)}] (p3) {};
\node[phase, fill=green!8, fit={(10.8,0) (13.9,-4.2)}] (p4) {};

% Phase titles
\node[above=0.15cm of p1.north, font=\fontsize{13}{15}\selectfont\bfseries, text=blue!70!black] {Ingest \& Normalize};
\node[above=0.15cm of p2.north, font=\fontsize{13}{15}\selectfont\bfseries, text=teal!70!black] {Filter \& Store};
\node[above=0.15cm of p3.north, font=\fontsize{13}{15}\selectfont\bfseries, text=orange!70!black] {Correlate \& Analyze};
\node[above=0.45cm of p4.north, anchor=south, font=\fontsize{13}{15}\selectfont\bfseries, text=green!60!black, align=center] {Respond \& Verify};

% Step nodes
\node[stepbox] (s1) at (1.55,-0.7) {\stepnum{1}\ Ingestion};
\node[stepbox] (s2) at (1.55,-1.85) {\stepnum{2}\ Mapping};
\node[stepbox] (s3) at (1.55,-3.0) {\stepnum{3}\ Enrichment};

\node[stepbox] (s4) at (5.15,-0.7) {\stepnum{4}\ Dedup};
\node[stepbox] (s5) at (5.15,-1.85) {\stepnum{5}\ Noise filter};
\node[stepbox] (s6) at (5.15,-3.0) {\stepnum{6}\ SQLite};

\node[stepbox] (s7) at (8.75,-0.7) {\stepnum{7}\ Correlation};
\node[stepbox] (s8) at (8.75,-1.85) {\stepnum{8}\ Investigation};
\node[stepbox] (s9) at (8.75,-3.0) {\stepnum{9}\ AI analysis};

\node[stepbox] (s10) at (12.35,-0.7) {\stepnum{10}\ Approval};
\node[stepbox] (s11) at (12.35,-1.85) {\stepnum{11}\ Execution};
\node[stepbox] (s12) at (12.35,-3.0) {\stepnum{12}\ Verify + Archive};

% Vertical arrows within phases
\foreach \a/\b in {s1/s2, s2/s3, s4/s5, s5/s6, s7/s8, s8/s9, s10/s11, s11/s12}{
  \draw[arrow] (\a) -- (\b);
}
% Horizontal arrows between phases
\draw[arrow] (s3.east) -- ++(0.35,0) |- (s4.west);
\draw[arrow] (s6.east) -- ++(0.35,0) |- (s7.west);
\draw[arrow] (s9.east) -- ++(0.35,0) |- (s10.west);
\end{tikzpicture}\end{adjustbox}
\caption{ARIA twelve-step intelligence pipeline grouped by phase}\label{fig:twelve-step}
\end{figure}

\section{Global ARIA Architecture}

ARIA has four cooperating layers. A frontend dashboard presents SOC operations; an API layer exposes the workflow over REST and WebSocket; a workflow layer contains the intelligence (pipeline, correlation, AI response, runtime, infrastructure, and operator logic); and an infrastructure layer connects to Elasticsearch, Redis, SQLite, Ansible, and the AI providers. Each layer depends only on the one below, keeping responsibilities clear and the system testable.
...
The platform is built around a small set of actors. Analysts and administrators interact through the dashboard; monitored assets, Elasticsearch, Redis, the AI provider, and Ansible targets are external participants coordinated by the workflow layer.
...
The complete catalogue of these interactions is the global use-case diagram in Figure~\ref{fig:global-usecase} (Chapter~\ref{ch:context}), mapping every actor to the use cases it triggers or serves. The same actors motivate the component view in Figure~\ref{fig:component}: the Next.js dashboard talks only to the FastAPI backend, which coordinates ingestion, AI, remediation, and asset services and the data and integration tier (Elasticsearch, Redis, SQLite, and the Ansible executor). The labelled ports on this diagram are ARIA's contract with its environment.

\umlfigure{aria_component_architecture.png}{ARIA component architecture and communication ports}{fig:component}

The technology choices follow directly from these needs: an asynchronous backend able to poll, transform, and serve data concurrently; a search source already in place; a fast operational store; an agentless automation tool; and a modern dashboard framework.
...
\raisebox{-0.22cm}{\includegraphics[width=1.0cm]{assets/logos/python.png}}\hspace{0.1cm}\raisebox{-0.22cm}{\includegraphics[width=1.0cm]{assets/logos/fastapi.png}}\hspace{0.15cm}The backend is a FastAPI application organized around domain packages. The route layer receives HTTP and WebSocket requests, the core layer centralizes shared infrastructure clients, the pipeline layer transforms external telemetry into SOC data, and the response layer manages investigations, AI output, automation, verification, and archive. This separation allows a background task to crash and restart without taking down the API server.
...
Figure~\ref{fig:backend-modules} expands these packages into the real module tree, showing how requests flow from API routers through the pipeline (poller, mappers, enrichment, data-usage) and the response layer (watcher, AI engines, remediation) into the SQLite models and shared \sourcefile{core/} clients. This is the concrete map a maintainer uses to locate logic.
...
The application is defined in \sourcefile{api/app.py}. It configures CORS, simple rate-limiting middleware, and a lifespan that initializes the database, then mounts domain routers: alerts, incidents, investigations, archives, and reports; assistant, adaptive, operator, runtime, and infrastructure; monitoring, pipeline, performance, dashboard, search, and IPS; assets, accounts, authentication, settings, and whitelist; plus WebSocket routes for real-time updates. Each domain has a dedicated file under \sourcefile{api/routes/}, keeping the API maintainable as it grows. The backend is wired to its environment through one configuration file; Listing~\ref{lst:backend-env} is a sanitized excerpt showing Elasticsearch, Redis, LLM, Ansible, and admin-secret settings, with secret values masked.

\begin{lstlisting}[style=aria,language=conf,caption={Backend configuration excerpt (sanitized .env)},label={lst:backend-env},
  emph={ELASTICSEARCH_URL,ELASTICSEARCH_USER,ELASTICSEARCH_PASSWORD,ELASTICSEARCH_USE_SSL,REDIS_HOST,REDIS_PORT,BACKEND_PORT,ARIA_ADMIN_SECRET,LLM_PROVIDER,LLM_MODEL,LLM_ENABLED,ANSIBLE_ENABLED,ANSIBLE_SSH_PORT,ANSIBLE_SSH_KEY},
...
\raisebox{-0.22cm}{\includegraphics[width=1.0cm]{assets/logos/nextjs.png}}\hspace{0.15cm}The dashboard is a Next.js, React, and TypeScript application using Tailwind CSS and shadcn/ui, SWR for data fetching and caching, Recharts for charts, and a WebSocket context for live updates. Its design goal is a dense, SOC-oriented interface: dark mode, severity badges, data tables, live indicators, page skeletons, and a command menu.

Architecturally, the frontend rests on three ideas. A dashboard shell holds authentication guard, sidebar, and global selected-asset context. A central API client attaches the JWT and admin-secret header and intercepts protected responses. Domain pages read and write through the SWR cache, synchronizing time filters and selected \codepath{asset_id} into their queries so the same component shows global or per-server data. Live events over WebSocket update the cache directly, so investigation, incident, performance, and health changes appear without manual refresh.
...
The dashboard pages form a navigation map (Figure~\ref{fig:frontend-nav}) that follows the SOC workflow: an \emph{Overview} group (dashboard, search), \emph{Security Operations} (alerts, incidents, investigations, IPS map, whitelist), \emph{Infrastructure \& Performance}, \emph{Runtime Security}, \emph{AI Intelligence} (assistant, operator), and \emph{System Management} (archives, settings). This grouping lets an analyst move from a high-level signal to a specific case in two or three clicks.
...
The local persistence layer uses SQLite with the SQLAlchemy asynchronous ORM \cite{sqlalchemy_async}. The model file \sourcefile{response/models.py} defines investigation lifecycle entities, alert snapshots, approvals, playbook runs, verification, archives, audit events, internal ARIA alerts, monitored assets, and whitelist entries. Storing this locally, rather than only in Elasticsearch, separates concerns: Elasticsearch holds raw, immutable evidence, while SQLite holds the \emph{operational state} of cases as they move through the workflow. An investigation's status, approval decision, Ansible run output, and verification result are operational facts that belong to ARIA, not to the source telemetry.
...
Figure~\ref{fig:class-model} gives the formal class diagram extracted from \sourcefile{response/models.py}. Each class carries its real attributes and is annotated with its workflow domain: \emph{Detection} (\codepath{Alert}, \codepath{Incident}, \codepath{AlertIncidentLink}), \emph{Remediation} (\codepath{Investigation}, \codepath{PlaybookApproval}, \codepath{PlaybookRun}, \codepath{FixVerification}, \codepath{OperatorRun}), \emph{Asset Scope} (\codepath{MonitoredAsset}, \codepath{AriaAccount}), and \emph{Audit/Archive} (\codepath{Archive}, \codepath{InvestigationAuditEvent}). The cardinalities express the central rule: one investigation owns at most one approval, one run, one verification, and one archive, but many context alerts and audit events, while a monitored asset scopes many alerts, incidents, investigations, and operator runs.

\umlfigure{class_domain_model.png}{Domain class diagram extracted from the backend models}{fig:class-model}

The same entities, viewed as the physical persistence schema, give the entity--relationship diagram in Figure~\ref{fig:er-model}. The schema is implemented in SQLite through SQLAlchemy async; it is sized for the single-node internship deployment and can be migrated to a production database later without changing the domain model, since the application depends on ORM entities rather than SQLite-specific features.
...
The alert pipeline reads raw events from Elasticsearch. The central design problem is to read \emph{new} events efficiently and exactly once, without busy-looping or missing data. ARIA solves this with cursor-based polling: for each source index pattern it keeps a per-source cursor (the last processed timestamp) in Redis, with a file fallback, and queries only events after that cursor. On the first run it uses a configurable lookback window.

The poller adapts its pace to workload. When a batch returns many alerts, it sleeps briefly and continues draining the backlog; when no new alerts are found, it returns to the configured polling interval. This avoids tight busy loops during quiet periods and unnecessary delay during active incidents. Each source (Wazuh, Suricata/Filebeat, Falco, Telegraf, and generic) is polled asynchronously, so a slow source does not block the others.
...
Raw security tools do not emit the same JSON schema, so the next stage normalizes every document into one common alert model. Correlation, deduplication, and AI analysis are only tractable if every alert exposes the same fields regardless of origin. Source-specific mappers perform this translation: \sourcefile{wazuh.py}, \sourcefile{suricata.py}, \sourcefile{falco.py} and \sourcefile{falco_runtime.py}, \sourcefile{filebeat.py} (including Suricata EVE events shipped through Filebeat), and \sourcefile{generic.py}. The result is a normalized alert with title, severity, source, timestamp, source and destination IPs, hostname, rule name, metadata, and indicators.

Once normalized, the alert is enriched with context an analyst would otherwise gather by hand:
...
Whatever the source, the pipeline produces one common shape. Listing~\ref{lst:normalized-alert} shows a normalized and enriched Wazuh SSH brute-force alert stored in the \codepath{alerts} table; the same fields are present for Suricata, Falco, or Filebeat alerts, making downstream correlation and AI analysis source-independent.

\begin{lstlisting}[style=aria,caption={Normalized and enriched alert (sanitized)},label={lst:normalized-alert},
  emph={source,source_id,severity,category,title,source_ip,hostname,rule_name,dedup_key,asset_id,iocs,alert_metadata,geoip,mitre,status,occurrence_count},
  emphstyle=\color{codekw}\bfseries]
{
  "source": "wazuh",
  "source_id": "Jx8...e1",
...
The decision logic is summarized in Figure~\ref{fig:act-correlation}: every active alert produces a \codepath{correlation_key}, which either extends an open incident or opens a new one; a scope check and response check then decide whether an investigation is created. The resulting incident follows the lifecycle in Figure~\ref{fig:incident-states} (\texttt{open -> investigating -> resolved -> archived}, with \codepath{resolved_by} recording whether closure was automatic or analyst-driven), and the hand-off to investigation is shown as a sequence in Figure~\ref{fig:seq-incident}.
...
An investigation carries a case from evidence to closure. Its lifecycle is an explicit state machine in \sourcefile{response/models.py}:

\begin{center}
\texttt{pending -> awaiting\_approval -> approved/declined -> running -> completed/failed -> archived}
\end{center}

Additional statuses exist for manual review, regeneration requests, completion warnings, and staged remediation. Making the lifecycle explicit is deliberate: every backend rule and UI action can be derived from the current state, removing ambiguity about what is allowed next and preventing execution on a case that has not been approved. The full state machine, including low-quality and regeneration branches, is given in Figure~\ref{fig:investigation-states}.
...
The response layer (\sourcefile{response/ai_engine/}, \sourcefile{response/runtime_ai_engine/}, \sourcefile{response/infrastructure_ai_engine/}) turns an incident into actionable investigation material. Its logic has three parts:

\begin{enumerate}[leftmargin=1cm]
\item \textbf{Context building.} Before any model is called, ARIA assembles a structured context with the alert timeline, source and destination indicators, MITRE information, behavioral history, risk score, and host information. Output quality depends almost entirely on input quality.
\item \textbf{Provider routing.} Prompt-builder modules and a multi-provider client route calls to the configured LLM and fall back gracefully when no provider is reachable, so the workflow never blocks on an external dependency.
\item \textbf{Structured output parsing.} The response is expected to contain a summary, narrative, risk assessment, and playbook. The parser extracts these into the \codepath{Investigation} model, and fallback parsing plus rule-based generation handle malformed LLM output.
\end{enumerate}

The consistent theme is that AI is treated as a fallible assistant whose output is always parsed, validated, and constrained, never trusted blindly.
...
ARIA separates \emph{proposing} a fix from \emph{executing} one. The AI may generate a playbook, but execution is gated by validation and approval. Ansible was chosen because it is an established, agentless automation tool that drives remote hosts over SSH \cite{ansible_docs}. In the workflow, a playbook is generated or edited, validated, written to a secure location with restrictive permissions, executed against the target host, and recorded in the database with its output and status. A natural-language prompt or raw AI output can never directly execute a command: the system must first produce a preview, a risk classification, or a concrete playbook, and an empty playbook is blocked outright.

The decision that governs whether a playbook runs is shown in Figure~\ref{fig:act-approval}: AI quality and safety tier are checked first, a risky tier requires an explicit admin override, and only analyst approval moves the case to \codepath{running}. Once running, execution itself is staged: the \codepath{PlaybookRun.current_phase} field walks through the phases in Figure~\ref{fig:remediation-states}, capturing a baseline, dry-running, applying containment and hardening, collecting forensics, and finally verifying, with any failure routed to a rollback path.
...
A command that returns success is not the same as a problem solved. After execution, the verifier re-checks evidence --- such as recurrence of the triggering events in Elasticsearch or the remote state of the host --- before a case may be considered resolved. This closes the loop honestly: a diagnostic-only case cannot be marked fixed, and a fix that did not stop the behavior is surfaced rather than hidden. Every sensitive transition is persisted as an audit event with actor, request context, timestamp, and authentication mode, so the path from alert to resolution is fully traceable.

\begin{figure}[H]
\centering
\begin{adjustbox}{max width=\textwidth}
\begin{tikzpicture}[
  block/.style={draw, rounded corners, align=center, minimum width=2.8cm, minimum height=1.4cm, fill=gray!8, font=\fontsize{17}{19}\selectfont},
  label/.style={align=center, font=\fontsize{13}{15}\selectfont},
  flow/.style={-Latex, line width=1.2pt},
  feedback/.style={-Latex, line width=1.2pt, dashed}
]
% Main components (equally sized boxes, generous spacing)
\node[block] (analyst) {\textbf{Analyst}};
\node[block, right=4.2cm of analyst] (api) {\textbf{FastAPI}};
\node[block, right=4.2cm of api] (ai) {\textbf{AI Engine}};
\node[block, right=4.2cm of ai] (ansible) {\textbf{Ansible}};
\node[block, right=4.2cm of ansible] (verifier) {\textbf{Verifier}};

% Primary workflow arrows (solid, horizontal, through box centers)
\draw[flow, <->] (analyst.east) -- (api.west);
\draw[flow] (api.east) -- (ai.west);
\draw[flow] (ai.east) -- (ansible.west);
\draw[flow] (ansible.east) -- (verifier.west);

% Labels in reserved spaces above the main arrows (never on the arrows)
\node[label, above=0.55cm] at ($(api.east)!0.5!(ai.west)$) {Operation summary};
\node[label, above=0.55cm] at ($(ai.east)!0.5!(ansible.west)$) {Generated playbook};
\node[label, above=0.55cm] at ($(ansible.east)!0.5!(verifier.west)$) {Execution result};

% Analyst <-> FastAPI bidirectional label (below the main arrow)
\node[label, below=0.55cm] at ($(analyst.east)!0.5!(api.west)$) {Approve / edit};

% Feedback / return flows underneath the main flow (dashed, non-overlapping)
\coordinate (ai-fb) at ($(ai.south)+(0,-1.1)$);
\coordinate (api-fb-ai) at ($(api.south |- ai-fb)$);
\draw[feedback] (ai.south) -- (ai-fb) -- (api-fb-ai) -- (api.south);
\node[label, below=0.35cm] at ($(ai.south)!0.5!(api.south |- ai-fb)$) {Summary + playbook};

\coordinate (verifier-fb) at ($(verifier.south)+(0,-2.5)$);
\coordinate (api-fb-v) at ($(api.south |- verifier-fb)$);
\draw[feedback] (verifier.south) -- (verifier-fb) -- (api-fb-v) -- (api.south);
\node[label, below=0.35cm] at ($(verifier.south)!0.5!(api.south |- verifier-fb)$) {Failure / unresolved state /\\retry feedback};
\end{tikzpicture}
\end{adjustbox}
\caption{AI investigation and remediation sequence (simplified)}
\end{figure}

The complete interaction, with real components and the admin-secret approval step, is the detailed sequence diagram in Figure~\ref{fig:seq-remediation}. It traces the path from automated context building and LLM generation, through safety classification and analyst approval, into staged Ansible execution on the monitored server, the Elasticsearch re-check by the verifier, and the final archive.
...
Figure~\ref{fig:asset-scoping} shows the same idea as a data-flow diagram centred on the \codepath{asset_scope} filter: every request carries the account role and \codepath{asset_id}, the monitored asset supplies per-source index patterns and host-identity fields, and the filter constrains every store (alerts, incidents, investigations, operator runs, metrics, and runtime events) to the authorized scope. A \codepath{super_admin} account may aggregate across all assets, while a \codepath{server_user} account is confined to its own. Each asset also has a lifecycle (Figure~\ref{fig:asset-states}): it is created, configured with its sources and Ansible settings, validated, and only then becomes \codepath{active}; failed validation lands in \codepath{partial}, \codepath{missing}, or \codepath{error} until it is reconfigured.
...
\begin{adjustbox}{max width=\textwidth}\begin{tikzpicture}[
  font=\small,
  box/.style={draw, rounded corners, align=center, text width=2.7cm, minimum height=1.0cm, fill=gray!8, font=\fontsize{13}{15}\selectfont},
  guard/.style={draw, rounded corners, align=center, text width=2.7cm, minimum height=1.0cm, fill=red!10, font=\fontsize{13}{15}\selectfont},
  exec/.style={draw, rounded corners, align=center, text width=2.7cm, minimum height=1.0cm, fill=orange!10, font=\fontsize{13}{15}\selectfont},
  verify/.style={draw, rounded corners, align=center, text width=2.7cm, minimum height=1.0cm, fill=green!10, font=\fontsize{13}{15}\selectfont},
  audit/.style={draw, rounded corners, align=center, text width=2.7cm, minimum height=1.0cm, fill=blue!10, font=\fontsize{13}{15}\selectfont},
  label/.style={align=center, font=\fontsize{11}{13}\selectfont, text=gray!70!black},
  arrow/.style={-Latex, line width=1.2pt},
  auditarrow/.style={-Latex, line width=1.0pt, dashed, gray!70}
]
% Main workflow nodes
\node[box] (ai) {\textbf{AI proposal}};
\node[guard, right=1.0cm of ai] (safety) {\textbf{Safety policy}};
\node[guard, right=1.0cm of safety] (approval) {\textbf{Approval gate}};
\node[guard, right=1.0cm of approval] (admin) {\textbf{Admin-secret gate}};
\node[exec, right=1.0cm of admin] (execnode) {\textbf{Ansible execution}};
\node[verify, right=1.0cm of execnode] (verifynode) {\textbf{Verification}};

% Audit output below
\node[audit, below=1.8cm of $(approval.east)!0.5!(admin.west)$] (auditnode) {\textbf{Audit log}};

% Main arrows
\draw[arrow] (ai) -- (safety);
\draw[arrow] (safety) -- (approval);
\draw[arrow] (approval) -- (admin);
\draw[arrow] (admin) -- (execnode);
\draw[arrow] (execnode) -- (verifynode);

% Phase labels above
\node[label, above=0.45cm] at ($(ai.east)!0.5!(safety.west)$) {proposed action};
\node[label, above=0.45cm] at ($(safety.east)!0.5!(approval.west)$) {risk check};
\node[label, above=0.45cm] at ($(approval.east)!0.5!(admin.west)$) {analyst decision};
\node[label, above=0.45cm] at ($(admin.east)!0.5!(execnode.west)$) {privilege check};
\node[label, above=0.45cm] at ($(execnode.east)!0.5!(verifynode.west)$) {fix applied};

% Audit arrows (dashed, from gates and execution)
\draw[auditarrow] (safety.south) -- ++(0,-0.4) -| (auditnode.north);
\draw[auditarrow] (approval.south) -- ++(0,-0.55) -| (auditnode.north);
\draw[auditarrow] (admin.south) -- ++(0,-0.7) -| (auditnode.north);
\draw[auditarrow] (execnode.south) -- ++(0,-0.85) -| (auditnode.north);
\draw[auditarrow] (verifynode.south) -- ++(0,-0.55) -| (auditnode.north);

\end{tikzpicture}\end{adjustbox}
\caption{Security control chain for AI-assisted remediation. Colors distinguish AI/analysis (gray), governance gates (red), execution (orange), verification (green), and audit output (blue).}
\end{figure}
