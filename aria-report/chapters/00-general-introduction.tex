...
This report is organized to follow the real engineering story of the project, that is, the order in which the solution was actually built from the ground up. \textbf{Chapter~1} presents the general context and project scope: the host organization, the cybersecurity and SOC background, the study and limits of the existing situation, the problem statement, the objectives, the proposed ARIA solution, and the working methodology and planning. \textbf{Chapter~2} describes the cloud environment and the central monitoring foundation: the Huawei Cloud infrastructure, the preparation of the main server, the network and communication design, and the installation, configuration, automation, and stability of each monitoring and security tool (Elasticsearch, Kibana, Wazuh, Suricata, Falco, Filebeat, Telegraf, and Redis) until real telemetry is verified inside Elasticsearch. \textbf{Chapter~3} explains the design and internal logic of the ARIA platform: its global, backend, and frontend architecture, its data model, and the reasoning behind each processing stage, from alert ingestion to normalization, deduplication, correlation, AI-assisted analysis, controlled remediation, verification, and asset scoping. \textbf{Chapter~4} presents the functional realization of ARIA, showing how each operational feature of the dashboard works on top of the collected data. \textbf{Chapter~5} covers multi-server onboarding and production deployment: how a new server is automatically prepared to send its data to the central stack, becomes a scoped asset inside ARIA, and how the whole solution is deployed and hardened for production. \textbf{Chapter~6} presents the validation and evaluation of the solution in the same build order: it validates the cloud environment and central monitoring stack first, confirms that real telemetry reaches Elasticsearch, and then validates the ARIA backend, frontend, individual features, multi-server scoping, runtime and remediation workflows, automated tests, and end-to-end scenarios, before listing the remaining limitations. The report ends with a general conclusion and appendices containing technical inventories, configuration extracts, and validation evidence.
