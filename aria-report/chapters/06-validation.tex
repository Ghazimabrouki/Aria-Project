...
Table~\ref{tab:fr-matrix} maps each high-level functional requirement from Chapter~1 to the validation evidence that supports it.

\begin{table}[H]
\centering
\caption{Functional requirement to validation evidence matrix}
\label{tab:fr-matrix}
\begin{tabularx}{\textwidth}{@{}p{0.10\textwidth}p{0.40\textwidth}X@{}}
\toprule
\textbf{FR} & \textbf{Requirement} & \textbf{Evidence} \\
\midrule
FR-01 & Poll multi-source alerts from Elasticsearch & Live ES indices (Fig.~\ref{fig:val-discover}, \ref{fig:ch2-es-idx}) \\
FR-02 & Normalize alerts into a common model & Chapter~3 pipeline diagrams and API alert records \\
FR-03 & Enrich with GeoIP and MITRE ATT\&CK & Chapter~4 IPS map and alert detail screenshots \\
FR-04 & Deduplicate and filter noise & Chapter~3 dedup/filter logic, project test records \\
FR-05 & Correlate alerts into incidents & Chapter~4 incident screenshots, API records \\
FR-06 & Generate AI investigations and playbooks & Chapter~4 investigation AI result, Listing~\ref{lst:ai-schema} \\
FR-07 & Approval workflow with admin-secret gate & Chapter~3 controls, Fig.~\ref{fig:val-rem} \\
FR-08 & Execute Ansible playbooks safely & Fig.~\ref{fig:val-rem} dry-run and hardening phases \\
FR-09 & Verify fixes and archive cases & Fig.~\ref{fig:val-rem} verification stage, archive screenshots \\
FR-10 & Multi-server asset scoping & Fig.~\ref{fig:val-assets}--\ref{fig:val-sources} \\
\bottomrule
\end{tabularx}
\end{table}
