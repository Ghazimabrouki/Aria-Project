...
restart_service_checked() {
  ...
  if ! timeout "$restart_timeout" systemctl restart "$service"; then
...
install_suricata() {
  ...
  if command -v suricata-update >/dev/null 2>&1; then
    timeout 120 suricata-update || warn "suricata-update failed or timed out; continuing with packaged/default rules."
  fi
  run_cmd systemctl daemon-reload
...
EOF
  ...
  if run_cmd systemctl list-unit-files --no-legend 2>/dev/null | awk '{print $1}' | grep -qx 'falco-modern-bpf.service' \
      || [[ -f "$(sandbox_path /lib/systemd/system/falco-modern-bpf.service)" ]] \
      || [[ -f "$(sandbox_path /usr/lib/systemd/system/falco-modern-bpf.service)" ]]; then
    FALCO_SERVICE="falco-modern-bpf.service"
  elif run_cmd systemctl list-unit-files --no-legend 2>/dev/null | awk '{print $1}' | grep -qx 'falco-bpf.service' \
      || [[ -f "$(sandbox_path /lib/systemd/system/falco-bpf.service)" ]] \
      || [[ -f "$(sandbox_path /usr/lib/systemd/system/falco-bpf.service)" ]]; then
    FALCO_SERVICE="falco-bpf.service"
  elif run_cmd systemctl list-unit-files --no-legend 2>/dev/null | awk '{print $1}' | grep -qx 'falco-kmod.service' \
      || [[ -f "$(sandbox_path /lib/systemd/system/falco-kmod.service)" ]] \
      || [[ -f "$(sandbox_path /usr/lib/systemd/system/falco-kmod.service)" ]]; then
    FALCO_SERVICE="falco-kmod.service"
  elif run_cmd systemctl list-unit-files --no-legend 2>/dev/null | awk '{print $1}' | grep -qx 'falco.service' \
      || [[ -f "$(sandbox_path /lib/systemd/system/falco.service)" ]] \
      || [[ -f "$(sandbox_path /usr/lib/systemd/system/falco.service)" ]]; then
...
install_falco_stack() {
  ...
  if ! run_cmd systemctl is-enabled "$FALCO_SERVICE" >/dev/null 2>&1; then
