...
Monitored VM setup completed.

Expected indexes:
  filebeat-${VM_NAME}-*
  wazuh-alerts-* with agent.name=${VM_NAME}
  falco-${VM_NAME}-*
  telegraf-${VM_NAME}-*
...
ARIA asset fields:
  ...
  host_name=${VM_NAME}
  monitored_asset=${VM_NAME}
  suricata_index_pattern=filebeat-${VM_NAME}-*
  suricata_host_name=${VM_NAME}
  falco_index_pattern=falco-${VM_NAME}-*
  telegraf_index_pattern=telegraf-${VM_NAME}-*
  remediation_enabled=false

Register asset in ARIA:
  curl -sS -X POST http://<aria-backend>:8001/api/assets \\
    -H 'Content-Type: application/json' \\
    -H 'Authorization: Bearer <admin-secret>' \\
    -d '{"name":"${VM_NAME}","host_name":"${VM_NAME}","monitored_asset":"${VM_NAME}","suricata_index_pattern":"filebeat-${VM_NAME}-*","suricata_host_name":"${VM_NAME}","falco_index_pattern":"falco-${VM_NAME}-*","telegraf_index_pattern":"telegraf-${VM_NAME}-*","remediation_enabled":false}'
